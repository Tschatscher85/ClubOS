            ...
            <Card>
              ...
              <CardContent>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex rounded-full overflow-hidden h-3 bg-muted">
                          {erwaermungProzent > 0 && (
                            <div className="bg-orange-400" style={{ width: `${erwaermungProzent}%` }} />
                          )}
                          {hauptteilProzent > 0 && (
                            <div className="bg-blue-500" style={{ width: `${hauptteilProzent}%` }} />
                          )}
                          {abschlussProzent > 0 && (
                            <div className="bg-green-500" style={{ width: `${abschlussProzent}%` }} />
                          )}
                        </div>
                        <div className="mt-2 space-y-0.5">
                          {einheit.hauptteil?.slice(0, 3).map((u, ui) => (
                            <p key={ui} className="text-[10px] text-muted-foreground truncate">{u.name}</p>
                          ))}
                          {(einheit.hauptteil?.length || 0) > 3 && (
                            <p className="text-[10px] text-muted-foreground">+{(einheit.hauptteil?.length || 0) - 3} weitere</p>
                          )}
                        </div>
                      </button>
                    );
                ...
                <div className="flex items-center gap-4 mt-3 text-xs text-muted-foreground">
                  <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full bg-orange-400" /> Erwaermung</div>
                  <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full bg-blue-500" /> Hauptteil</div>
                  <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full bg-green-500" /> Abschluss</div>
                </div>
              </CardContent>
            </Card>
          )}
